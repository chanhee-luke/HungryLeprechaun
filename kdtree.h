...
namespace kdt {
	...
	*/
	...
	public:
		...
		*/
		std::vector<int> knnSearch(const PointT& query, int k, std::function<bool (PointT)> filter = NULL) const {
			KnnQueue queue(k);
			knnSearchRecursive(query, root_, queue, k, filter);
		...
		*/
		std::vector<int> radiusSearch(const PointT& query, double radius, std::function<bool (PointT)> filter = NULL) const {
			std::vector<int> indices;
			radiusSearchRecursive(query, root_, indices, radius, filter);
			return indices;
		}
	...
	private:
		...
		*/
		void knnSearchRecursive(const PointT& query, const Node* node, KnnQueue& queue, int k, std::function<bool (PointT)> filter = NULL) const {
			if (node == nullptr)
				return;
			...
			const double dist = distance(query, train);
			if(filter == NULL || filter(train))
				queue.push(std::make_pair(dist, node->idx));

			const int axis = node->axis;
			const int dir = query[axis] < train[axis] ? 0 : 1;
			knnSearchRecursive(query, node->next[dir], queue, k, filter);
		...
		/** @brief Searches neighbors within radius.
		*/
		void radiusSearchRecursive(const PointT& query, const Node* node, std::vector<int>& indices, double radius, std::function<bool (PointT)> filter = NULL) const {
			if (node == nullptr)
				return;
			...
			const double dist = distance(query, train);
			if (dist < radius && (filter == NULL || filter(train)))
				indices.push_back(node->idx);

			const int axis = node->axis;
			const int dir = query[axis] < train[axis] ? 0 : 1;
			radiusSearchRecursive(query, node->next[dir], indices, radius, filter);
